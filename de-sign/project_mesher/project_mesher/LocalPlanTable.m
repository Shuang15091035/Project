...
- (void)setRecord:(id)record word:(NSString *)word columnIndex:(int)columnIndex {
    ...
    if (columnIndex == [self getColumnIndexById:LocalPlanTableColumnId_Id]) {
        ...
        r.name = word;
    } else if (columnIndex == [self getColumnIndexById:LocalPlanTableColumnId_Type]) {
        switch (mFileType) {
            case LocalPlanTableFileTypeDocument: {
                break;
            }
            case LocalPlanTableFileTypeAsset: {
                r.type = [Data parsePlanType:word];
                break;
            }
        }
...
- (int)getColumnIdByName:(NSString *)columnName {
    ...
    if ([NSString is:columnName equalsTo:@"类型"]) {
        return LocalPlanTableColumnId_Type;
    }
    ...
    switch (mFileType) {
        case LocalPlanTableFileTypeDocument: {
            ...
            if (columnIndex == LocalPlanTableColumnId_Id) {
                ...
                return r.name;
            } else if (columnIndex == LocalPlanTableColumnId_Type) {
                return [Data planTypeToString:r.type];
