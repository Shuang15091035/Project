...
@interface ArchitureEdit () <JIOnResourceLoadingListener> {
    JWRelativeLayout *lo_WallEdit;
    UIImageView *centerImage;
    JWCollectionView *cv_color;
    WallAdapter *mWallAdapter;
    JWLinearLayout *lo_camera;
    
    JWRelativeLayout *lo_ArchitureDetails;
    UIImageView* img_details_image;
    JWImageOptions* opt_img_details_image;
    JWRadioViewGroup *rg_option;
    UIButton *btn_change_texture;
    JWCollectionView *cv_texture;
    JWRelativeLayout *lo_texture;
    ...
    id<JIViewTag> mArchitureEditMenu;
    id<JIGameObject> ViewTagObject;
    
    id<JITexture> texture;
    id<JIMaterial> material;
    id<JIMaterial> tempMaterial;
    JWAsyncResult *result; // 异步载入贴图的返回信息 可用于取消异步加载
...
- (UIView *)onCreateView:(UIView *)parent {
    ...
    lo_WallEdit = [JWRelativeLayout layout];
    lo_WallEdit.tag = R_id_lo_wall_edit;
    lo_WallEdit.layoutParams.width = mModel.gameViewWidth;
    lo_WallEdit.layoutParams.height = JWLayoutMatchParent;
    lo_WallEdit.backgroundColor = [UIColor clearColor];
    lo_WallEdit.layoutParams.alignment = JWLayoutAlignParentBottomLeft;
    ...
    cv_color = [[JWCollectionView alloc] initWithFrame:CGRectZero collectionViewLayout:wallLayout];
    ...
    cv_color.layoutParams.alignment = JWLayoutAlignParentBottomLeft;
    ...
    __block JWCollectionView *cv_color_b = cv_color;
    __block id<IMesherModel> model = mModel;
    __block JWAsyncResult *result_b = result;
    __block JWRelativeLayout *lo_WallEdit_b = lo_WallEdit;
    cv_color.onScrolled = ^(id<JIAdapterView> view, NSUInteger firstVisibleItem, NSUInteger visibleItemCount, NSUInteger totalItemCount) { // 滚动结束后调用
        ...
        it.Id = item.Id;
        info.item = it;
        ObjectExtra *extra = model.borderObject.extra;
        if (extra == nil) {
            extra = [ObjectExtra new];
        }
        extra.indexPathRow = i.row;
        model.borderObject.extra = extra;
    };
    cv_color.onItemSelected = ^(NSUInteger position, id item, BOOL selected) {
        ...
        extra.indexPathRow = position;
        model.borderObject.extra = extra;
    };
    cv_color.onScroll = ^(id<JIAdapterView> view) {//滚动中调用
        CGPoint cv_center = CGPointMake(mModel.gameViewWidth/2, [UIScreen mainScreen].bounds.size.height - [MesherModel uiHeightBy:75.0f]); // 获取屏幕水平中心的某一个点
        CGPoint inView = [lo_WallEdit_b convertPoint:cv_center toView:cv_color_b];
//        CGPoint inView = CGPointMake(mModel.gameViewWidth/2, cv_color_b.frame.size.height/2);
    ...
    centerImage.layoutParams.alignment = JWLayoutAlignParentBottom | JWLayoutAlignCenterHorizontal;
...
#pragma mark item上的按钮
    lo_object_menu_container = [JWFrameLayout layout];
    lo_object_menu_container.layoutParams.width = JWLayoutMatchParent;
    lo_object_menu_container.layoutParams.height = JWLayoutMatchParent;
    ...
    //lo_object_menu.layoutParams.alignment = JWLayoutAlignCenterInParent;
    ...
    btn_brush.layoutParams.width = JWLayoutWrapContent;
    btn_brush.layoutParams.height = JWLayoutWrapContent;
    ...
    btn_details.layoutParams.width = JWLayoutWrapContent;
    btn_details.layoutParams.height = JWLayoutWrapContent;
...
#pragma mark details
    lo_ArchitureDetails = [JWRelativeLayout layout];
    lo_ArchitureDetails.tag = R_id_lo_architure_details;
    lo_ArchitureDetails.layoutParams.width = [self uiWidthBy:521.0f];
    lo_ArchitureDetails.layoutParams.height = JWLayoutMatchParent;
    lo_ArchitureDetails.layoutParams.alignment = JWLayoutAlignParentRight;
    ...
    bg_architureDetails.layoutParams.width = [self uiWidthBy:521.0f];
    bg_architureDetails.layoutParams.height = JWLayoutMatchParent;
    ...
    img_details_image.layoutParams.alignment = JWLayoutAlignCenterHorizontal;
    ...
    btn_change_texture.layoutParams.height = JWLayoutMatchParent;
    btn_change_texture.layoutParams.alignment = JWLayoutAlignParentLeft;
    ...
    [btn_change_texture setBackgroundImage:[UIImage imageByResourceDrawable:@"btn_style_n"] forState:UIControlStateNormal];
    [btn_change_texture setBackgroundImage:[UIImage imageByResourceDrawable:@"btn_style_p"] forState:UIControlStateSelected];
...
#pragma mark 贴图替换
    lo_texture = [JWRelativeLayout layout];
    lo_texture.layoutParams.width = JWLayoutMatchParent;
    if ([UIDevice currentDevice].type == UIDeviceTypeIPad){
    ...
    lo_texture.layoutParams.alignment = JWLayoutAlignParentBottom | JWLayoutAlignCenterHorizontal;
    [lo_ArchitureDetails addSubview:lo_texture];
    
    cv_texture = [JWCollectionView collectionView];
    cv_texture.layoutParams.width = [MesherModel uiWidthBy:480.0f];
    cv_texture.layoutParams.height = JWLayoutMatchParent;
    cv_texture.layoutParams.marginLeft = [MesherModel uiWidthBy:30.0f];
    cv_texture.numColumns = 3;
    cv_texture.backgroundColor = [UIColor whiteColor];
...
- (void)onStateEnter:(NSDictionary *)data {
    ...
    centerImage.image = nil;
    id<JIFile> tempImage = [architures[0] preview];
    [[JWCorePluginSystem instance].imageCache getBy:tempImage options:nil async:NO onGet:^(UIImage *image) {
        centerImage.image = image;
    }];
...
#pragma mark viewTag的设置
    if (ViewTagObject == nil) {
        //用一个obg承载一个viewtag
        ViewTagObject = [mModel.currentContext createObject];
        ViewTagObject.queryMask = SelectedMaskCannotSelect; // 设置不可点击 防止误操作
        ViewTagObject.parent = mModel.currentScene.root;// 场景内的物件需要受到场景管理 设置parent为当前场景
    }
    if (mArchitureEditMenu == nil) {
        mArchitureEditMenu = [mModel.currentContext createViewTag];
        mArchitureEditMenu.view = lo_object_menu;
        mArchitureEditMenu.viewOffset = CGPointMake(0, -100);
        mArchitureEditMenu.onChange = (^CGPoint (CGPoint position) {
            ...
            float minY = lo_WallEdit.frame.size.height/5;
            float maxY = lo_WallEdit.frame.size.height/5*4;
    ...
    [ViewTagObject addComponent:mArchitureEditMenu];
//    [mModel.borderObject addComponent:mArchitureEditMenu];
    JCVector3 min = mModel.borderObject.transformBounds.min;
    JCVector3 max = mModel.borderObject.transformBounds.max;
    JCVector3 borderCenter = JCVector3Make(min.x + (max.x-min.x)/2, min.y + (max.y-min.y)/2, min.z + (max.z-min.z)/2); // 取出 墙体 或 地板 的中心点
    [ViewTagObject.transform setPosition:borderCenter inSpace:JWTransformSpaceWorld]; // 将viewtag的载体obj放在目标位置
...
- (void)drawBorder {
    ...
    if (extra.archBorder != nil) {
        id<JIGameObject> borderObj = extra.archBorder;
        ...
        for (int i = 0; i < path.count; i += 3) {
            JCFloat x = [path[i] floatValue];
            JCFloat y = [path[i+1] floatValue];
            JCFloat z = [path[i+2] floatValue];
            JWVector3 *p = [[JWVector3 alloc] initWithX:x Y:y Z:z];
            [allPoints add:p];
        }
        BOOL isX = NO;
        BOOL isY = NO;
        ...
        JWVector3 *p1 = allPoints[0];
        JWVector3 *p2 = allPoints[1];
        JWVector3 *p3 = allPoints[2];
        ...
        JWVector3 *min = allPoints[0];
        for (int i = 1; i < allPoints.count; i++) {
            JWVector3 *p = allPoints[i];
        ...
        id<JIFile> file = [JWFile fileWithName:objName content:nil];
        id<JIMesh> mesh = (id<JIMesh>)[mModel.currentContext.meshManager createFromFile:file];
        [mesh makeWithNumVertices:allPoints.count numIndices:0];
        [mesh beginOperation:JCRenderOperationLineLoop update:NO];
        // JCUShort i = 0;
        for (JWVector3* pos in allPoints) {
        ...
        [mesh end];
        [mesh load];
        id<JIMeshRenderer> meshRenderer = [mModel.currentContext createMeshRenderer];
        meshRenderer.mesh = mesh;
        id<JIGameObject> borderObj = [mModel.currentContext createObject];
        borderObj.Id = objName;
        [borderObj addComponent:meshRenderer];
        borderObj.parent = mModel.borderObject;
        borderObj.visible = YES;
        borderObj.queryMask = SelectedMaskCannotSelect;
        extra.archBorder = borderObj;
...
- (void)loadTextureByFile:(id<JIFile>)file {
    if (file != nil) {
        ...
        if (texture.isValid) {
            ...
            id<JIFile> materialFile = [JWFile fileWithType:JWFileTypeMemory path:name];
            material = (id<JIMaterial>)[mModel.currentContext.materialManager createFromFile:materialFile];
...
- (BOOL)onTouchUp:(NSSet *)touches withEvent:(UIEvent *)event {
    ...
    switch (touch.view.tag) {
        case R_id_btn_brush: {
#pragma mark 切换镜头 暂时不用
//            ItemInfo *info = [Data getItemInfoFromInstance:mModel.borderObject];
//            
//            JCVector3 normal = JCVector3Make(info.dx.floatValue, 0.0f, info.dz.floatValue);
//            normal = JCVector3Normalize(&normal);
//            
//            JCVector3 min = mModel.borderObject.transformBounds.min;
//            JCVector3 max = mModel.borderObject.transformBounds.max;
//            JCVector3 position = JCVector3Make(min.x + (max.x-min.x)/2, min.y + (max.y-min.y)/2, min.z + (max.z-min.z)/2);
//            //JWEditorCameraPrefab *archCamera = [mModel.photographer changeToArchEditCamera:1000];
//            //先设置摄像机
//            JWEditorCameraPrefab* archCamera = mModel.photographer.archEditCamera;
//            
//            if (info.type == ItemTypeWall) {
//                [archCamera.root.transform setPositionV:position inSpace:JWTransformSpaceWorld];
//                JCVector3 camera_Z = [archCamera.root.transform zAxisInSpace:JWTransformSpaceParent];
//                camera_Z.y = 0.0f;
//                camera_Z = JCVector3Normalize(&camera_Z);
//                float d = JCVector3DotProductv(&normal, &camera_Z);
//                float radians = acosf(d);
//                float degrees = JCRad2Deg(radians); // 旋转的角度
//                JCVector3 cn = JCVector3CrossProductv(&camera_Z, &normal);
//                if (cn.y < 0.0f) {
//                    degrees = -degrees;
//                }
...
//            
//            ObjectExtra *extra = mModel.borderObject.extra;
//            if (extra.indexPathRow != 0) {
//                [cv_color selectItemAtPosition:extra.indexPathRow animated:NO scrollPosition:UICollectionViewScrollPositionCenteredHorizontally];
//            }
//            
//            mModel.photographer.cameraEnabled = YES;
//            [archCamera.camera.host removeComponent:archCamera.camera.host.behaviour];
