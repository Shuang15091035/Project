...
- (void)setRecord:(id)record word:(NSString *)word columnIndex:(int)columnIndex {
    ...
    if (columnIndex == [self getColumnIndexById:LocalItemTableColumnId_Id]) {
        ...
        r.previewSmall = [JWFile fileWithBundle:mBundle path:[JWAssetsBundle nameForPath:word]];
        r.previewSmall.extra = @(1);
        r.previewBig = [JWFile fileWithBundle:mBundle path:[JWAssetsBundle nameForPath:word]];
